...
def get_data(N=50, D_X=3, sigma_obs=0.2, N_test=500, N_val=20, gap=True, seed=0):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 1000 == 0:
            ...
            with torch.no_grad():  # Disable gradient computation
                _, _, _, _, X_val, Y_val = get_data(N=150, D_X=3, N_test=500, sigma_obs=0.05, seed=np.random.randint(0, 1000))
                val_inference = []
                for i in range(20):
