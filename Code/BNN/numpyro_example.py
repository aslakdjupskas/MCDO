...
def main(args):
    ...
    plt.figure()
    plt.hist(samples["w1"][:,0,0]) # Distribution og 1st weight
    plt.show()
    ...
    ax.plot(X[:, 1], Y[:, 0], "r.")
    for _ in range(5):
        ax.plot(X_test[:, 1], np.random.multivariate_normal(mean_prediction, cov_matrix), alpha=0.1, color='black')  # Plot the posterior samples
    ...
    plt.show()
    plt.savefig("bnn_plot1.pdf")


if __name__ == "__main__":
    assert numpyro.__version__.startswith("0.16.1")
    parser = argparse.ArgumentParser(description="Bayesian neural network example")
    parser.add_argument("-n", "--num-samples", nargs="?", default=50, type=int)
