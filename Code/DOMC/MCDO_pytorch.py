...
class MCVariationalDense(nn.Module):
    def __init__(self, n_in, n_out, model_prob, model_lam, layer):
    ...
    def forward(self, X):
        if not self.training and self.layer in [1, 2, 3]:
...
class MCVariationalNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, model_prob, model_lam):
        ...
        self.layer1 = MCVariationalDense(input_size, hidden_size, model_prob, model_lam, layer=1)
        self.layer2 = MCVariationalDense(hidden_size, hidden_size, model_prob, model_lam, layer=2)
        self.layer3 = MCVariationalDense(hidden_size, hidden_size, model_prob, model_lam, layer=3)
        self.layer4 = MCVariationalDense(hidden_size, output_size, model_prob, model_lam, layer=4)
...
for p, model_prob in enumerate([0.05, 0.1]):
    for j, hidden_size in enumerate([12, 64]):


        #hidden_size = 100
        # model_prob  = 0.01   # Dropout probability
...
plt.savefig("MCDO/Code/DOMC/plots/MCDO_pytorch_hiddendiffer.pdf")
print("Done")

# Plot results
plt.figure(figsize=(8, 5))
plt.plot(X[:, 1], Y, "r.", label="Train Data", alpha=0.5)
plt.plot(X_test, mean_pred, label="MC Mean Prediction", color="blue")
for i in range(150):
    plt.plot(X_test, predictions[5000+i], color="gray", alpha=0.05)
plt.fill_between(
    X_test,
    (mean_pred - 2 * std_pred).flatten(),
    (mean_pred + 2 * std_pred).flatten(),
